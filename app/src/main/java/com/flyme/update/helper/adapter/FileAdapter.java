package com.flyme.update.helper.adapter;
...
public class FileAdapter extends BaseAdapter {
    ...
    private FileDialogUtils dialog;

    public FileAdapter(FileDialogUtils dialog, Activity activity, ArrayList<FileBean> fileList) {
        super();
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = layoutInflater.inflate(com.kongzue.filedialog.R.layout.item_dialogx_file_list, null);
        }
        ...
        String fileName = fileList.get(position).Name;
        ...
        if ("...".equals(fileName)) {
            ...
            viewHolder.imgIcon.setImageResource(com.kongzue.filedialog.R.drawable.ic_dialogx_filedialog_back);
            viewHolder.btnHaveChild.setVisibility(View.GONE);
        } else {
            if (fileName.startsWith("/")) {
                viewHolder.txtFileName.setText(fileName.substring(1));
                viewHolder.imgIcon.setImageResource(com.kongzue.filedialog.R.drawable.ic_dialogx_filedialog_folder);
                ...
                if (dialog.getSelectPathList() != null && dialog.getSelectPathList().contains(itemPath)) {
                    viewHolder.imgIcon.setImageResource(com.kongzue.filedialog.R.drawable.ic_dialogx_filedialog_select);
                    viewHolder.txtFileName.getPaint().setFakeBoldText(true);
                } else {
                    viewHolder.imgIcon.setImageResource(com.kongzue.filedialog.R.drawable.ic_dialogx_filedialog_file);
                }
                viewHolder.btnHaveChild.setVisibility(View.GONE);
            }
        }
    ...
    private String getFileDate(Context context, File file) {
        ...
        SimpleDateFormat dateFormat = new SimpleDateFormat(context.getString(com.kongzue.filedialog.R.string.dialogx_filedialog_default_date_format));

        if (fileCal.get(Calendar.YEAR) == today.get(Calendar.YEAR)
                && fileCal.get(Calendar.DAY_OF_YEAR) == today.get(Calendar.DAY_OF_YEAR)) {
            SimpleDateFormat timeFormat = new SimpleDateFormat(context.getString(com.kongzue.filedialog.R.string.dialogx_filedialog_default_date_format_today));
            ...
            SimpleDateFormat timeFormat = new SimpleDateFormat(context.getString(com.kongzue.filedialog.R.string.dialogx_filedialog_default_date_format_yesterday));
            return timeFormat.format(lastModified);
        } else {
    ...
    class ViewHolder {
        ...
        public ViewHolder(View convertView) {
            imgIcon = convertView.findViewById(com.kongzue.filedialog.R.id.img_icon);
            txtFileName = convertView.findViewById(com.kongzue.filedialog.R.id.txt_fileName);
            txtDate = convertView.findViewById(com.kongzue.filedialog.R.id.txt_date);
            btnHaveChild = convertView.findViewById(com.kongzue.filedialog.R.id.btn_have_child);
        }
    }
}
